...
const SHORTCUTS: ShortcutItem[] = [
  ...
  { category: 'Viewport & Navigation', keys: ['MMB'], desc: 'Orbit 3D viewport' },
  {
    ...
    keys: ['Alt', 'LMB'],
    desc: 'Orbit 3D viewport (alternate)'
  },
  ...
  { category: 'Tools & Radial Menu', keys: ['L'], desc: 'Line tool (straight surface strokes)' },
  {
    ...
    keys: ['Shift', 'Click'],
    desc: 'Draw straight line with Brush'
  },
  ...
  {
    category: 'Brush Controls & Adjustments',
    keys: ['Shift', 'R'],
    desc: 'Rotate brush angle (-15°)'
  },
  {
    category: 'Brush Controls & Adjustments',
    keys: ['X'],
    desc: 'Swap Mask B/W or toggle Solid/Texture'
  },
  {
    category: 'Brush Controls & Adjustments',
    keys: ['Alt', 'X'],
    desc: 'Cycle symmetry axis (Off → X → Y → Z)'
  },

  // Geometry & Face Selection
  {
    category: 'Geometry & Face Selection',
    keys: ['Ctrl', 'A'],
    desc: 'Select all faces on active piece'
  },
  { category: 'Geometry & Face Selection', keys: ['Ctrl', 'D'], desc: 'Deselect all faces' },
  {
    category: 'Geometry & Face Selection',
    keys: ['Escape'],
    desc: 'Clear face selection / cancel modal'
  },
  { category: 'Geometry & Face Selection', keys: ['Ctrl', 'I'], desc: 'Invert face selection' },
  {
    category: 'Geometry & Face Selection',
    keys: ['Ctrl', 'Drag'],
    desc: 'Paint-select multiple faces'
  },
  {
    category: 'Geometry & Face Selection',
    keys: ['Double LMB'],
    desc: 'Select connected UV island (in Face tool)'
  },

  // Multi-Piece Models
  { category: 'Multi-Piece Models', keys: ['Tab'], desc: 'Next model piece / texture set' },
  {
    category: 'Multi-Piece Models',
    keys: ['Shift', 'Tab'],
    desc: 'Previous model piece / texture set'
  },
  {
    category: 'Multi-Piece Models',
    keys: ['Double LMB'],
    desc: 'Select piece under cursor (in paint tools)'
  },
  ...
  {
    category: 'History & Application',
    keys: ['Ctrl', 'Shift', 'E'],
    desc: 'Open Export Textures wizard'
  },
  {
    category: 'History & Application',
    keys: ['Ctrl', 'E'],
    desc: 'Open Export Textures wizard (alternate)'
  },
  {
    category: 'History & Application',
    keys: ['Ctrl', 'Z'],
    desc: 'Undo last stroke or layer edit'
  },
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex items-center gap-1.5 p-1 bg-zinc-950/70 border border-zinc-850 rounded-xl overflow-x-auto select-none">
          ...
          >
            <KeyboardIcon
              size={14}
              class={activeTab() === 'shortcuts' ? 'text-blue-400' : 'text-zinc-500'}
            />
            <span>Shortcuts</span>
          </button>
          ...
          >
            <CompassIcon
              size={14}
              class={activeTab() === 'workflow' ? 'text-blue-400' : 'text-zinc-500'}
            />
            <span>Workflow</span>
          </button>
          ...
          >
            <BrushIcon
              size={14}
              class={activeTab() === 'tools' ? 'text-blue-400' : 'text-zinc-500'}
            />
          ...
          >
            <PaletteIcon
              size={14}
              class={activeTab() === 'pbr' ? 'text-blue-400' : 'text-zinc-500'}
            />
          ...
          >
            <LayersIcon
              size={14}
              class={activeTab() === 'layers' ? 'text-blue-400' : 'text-zinc-500'}
            />
                      ...
                      <For each={items}>
                        {(item) => (
                          <div class="flex items-center justify-between gap-2 py-0.5 text-xs">
                            ...
                            <span class="text-zinc-400 text-right text-[11px] leading-snug">
                              {item.desc}
                            </span>
                          </div>
                        )}
            ...
            <Show when={Object.keys(groupedCategories()).length === 0}>
              <div class="p-8 text-center bg-zinc-950/40 border border-zinc-800 rounded-xl">
                <p class="text-xs text-zinc-400">
                  No shortcuts matched &ldquo;{searchQuery()}&rdquo;
                </p>
                <button
                  type="button"
          ...
          <div class="flex flex-col gap-4 text-xs text-zinc-300 leading-relaxed">
            ...
            <div class="p-4 bg-gradient-to-r from-blue-950/40 via-zinc-900/60 to-zinc-950/60 border border-blue-500/20 rounded-xl flex flex-col gap-2">
              ...
              <p class="text-zinc-300 text-xs">
                MeshCoat is a high-performance 3D surface painting suite designed for texturing low,
                mid, and high-poly 3D models directly in your viewport. It projects strokes from the
                camera onto the 3D surface, painting across UV seams smoothly without manual 2D
                texture unfolding.
              </p>
            </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                  <div class="flex items-center gap-2 font-semibold text-zinc-200">
                    <span class="w-5 h-5 rounded-full bg-blue-600/30 text-blue-400 flex items-center justify-center text-[10px] font-mono font-bold">
                      1
                    </span>
                    <h4>Import 3D Model</h4>
                  </div>
                  <p class="text-[11px] text-zinc-400">
                    Load any <code class="text-blue-400">.obj</code>,{' '}
                    <code class="text-blue-400">.gltf</code>, or{' '}
                    <code class="text-blue-400">.glb</code> asset via File &gt; Open 3D Model.
                    Models can contain single or multiple meshes with UV0 coordinates.
                  </p>
                </div>
                ...
                <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                  ...
                  <p class="text-[11px] text-zinc-400">
                    Choose from 512px to 4096px texture resolution. For multi-piece models, each
                    mesh piece maintains its own independent texture set and layer stack.
                  </p>
                </div>
                ...
                <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                  ...
                  <p class="text-[11px] text-zinc-400">
                    Use Brush, Line, Stamped Decals, or Stencils. Strokes project in tangent-space,
                    automatically bleeding across UV borders to prevent seam gaps.
                  </p>
                </div>
                ...
                <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                  ...
                  <p class="text-[11px] text-zinc-400">
                    Paint Base Color, Roughness, Metalness, Normal, and Ambient Occlusion. Inspect
                    individual channels in the top bar with hotkeys.
                  </p>
                </div>
                ...
                <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                  ...
                  <p class="text-[11px] text-zinc-400">
                    Layer blend modes (Multiply, Screen, Overlay, etc.), non-destructive layer
                    masks, and procedural edge wear generator for realistic weathering.
                  </p>
                </div>
                ...
                <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                  ...
                  <p class="text-[11px] text-zinc-400">
                    Export PNG maps or packed ORM textures (AO, Roughness, Metalness). MeshCoat
                    automatically saves recovery backups every 60 seconds.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
              ...
              <p class="text-zinc-400 text-[11px]">
                When painting on complex geometry, 2D UV seams often cut across visible surfaces
                (such as across character faces or curved mechanical parts). MeshCoat executes
                screen-to-surface raycasting combined with an occlusion depth buffer. This ensures
                paint only applies to front-facing geometry without bleeding onto backfaces or
                hidden shells, and texel dilation keeps seam edges seamless at any camera angle.
              </p>
            </div>
          </div>
        </Show>
        ...
        <Show when={activeTab() === 'tools'}>
          <div class="flex flex-col gap-3 text-xs text-zinc-300">
            <p class="text-[11px] text-zinc-400">
              MeshCoat provides 8 specialized viewport tools, along with Screen Stencils, Symmetry
              Mirroring, and the Radial Pie Menu.
            </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="p-1.5 rounded-lg bg-blue-600/20 text-blue-400">
                      <BrushIcon size={16} />
                    </div>
                ...
                <p class="text-[11px] text-zinc-400">
                  Projects smooth circular or textured dabs onto 3D geometry. Customize Radius,
                  Opacity, Hardness (edge falloff), Flow (dab rate), Spacing, Jitter, and Scatter.
                  Supports tiled pattern textures with adjustable scale.
                </p>
              </div>
              ...
              <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="p-1.5 rounded-lg bg-purple-600/20 text-purple-400">
                      <LineIcon size={16} />
                    </div>
                ...
                <p class="text-[11px] text-zinc-400">
                  Draws straight strokes between two points. Raycasts along the screen-space path to
                  hug curved surfaces without cutting through internal geometry. Can also be invoked
                  with <Kbd size="xs">Shift + Click</Kbd> while using the brush.
                </p>
              </div>
              ...
              <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="p-1.5 rounded-lg bg-red-600/20 text-red-400">
                      <EraserIcon size={16} />
                    </div>
                ...
                <p class="text-[11px] text-zinc-400">
                  Non-destructively reduces alpha on the active layer. Allows carving out highlights
                  or cleaning up edges without affecting layers underneath.
                </p>
              </div>
              ...
              <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="p-1.5 rounded-lg bg-emerald-600/20 text-emerald-400">
                      <StampIcon size={16} />
                    </div>
                ...
                <p class="text-[11px] text-zinc-400">
                  Projects decals, logos, and emblems onto surfaces. Shows a live on-surface preview
                  orientation ring. Adjust size, angle, and opacity before stamping down.
                </p>
              </div>
              ...
              <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="p-1.5 rounded-lg bg-amber-600/20 text-amber-400">
                      <FillIcon size={16} />
                    </div>
                ...
                <p class="text-[11px] text-zinc-400">
                  Floods geometry with color or repeating texture. When face selection is active, it
                  fills only the selected faces; otherwise it floods the whole active mesh piece.
                </p>
              </div>
              ...
              <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="p-1.5 rounded-lg bg-cyan-600/20 text-cyan-400">
                ...
                <p class="text-[11px] text-zinc-400">
                  Samples colors directly from the 3D surface with a precision magnification loupe
                  preview showing RGB hex values. Also samples active PBR channel data.
                </p>
              </div>
              ...
              <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="p-1.5 rounded-lg bg-indigo-600/20 text-indigo-400">
                      <CubeIcon size={16} />
                    </div>
                ...
                <p class="text-[11px] text-zinc-400">
                  Isolates sub-mesh triangles. <Kbd size="xs">Double-click</Kbd> selects a connected
                  UV island. <Kbd size="xs">Ctrl + Drag</Kbd> paint-selects faces. Constrains all
                  paint operations exclusively to selected geometry.
                </p>
              </div>
              ...
              <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="p-1.5 rounded-lg bg-pink-600/20 text-pink-400">
                ...
                <p class="text-[11px] text-zinc-400">
                  Filters texels directly on the active layer in premultiplied alpha space. Modes
                  include <span class="text-zinc-200 font-medium">Blur</span> (soften seams),{' '}
                  <span class="text-zinc-200 font-medium">Sharpen</span> (enhance details),{' '}
                  <span class="text-zinc-200 font-medium">Smudge</span> (blend strokes), and{' '}
                  <span class="text-zinc-200 font-medium">Pixelate</span>.
                </p>
              </div>
              ...
              <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="p-1.5 rounded-lg bg-teal-600/20 text-teal-400">
                      <ImagesIcon size={16} />
                    </div>
                ...
                <p class="text-[11px] text-zinc-400">
                  Floats a 2D guide image over the viewport. Use <Kbd size="xs">Alt + LMB</Kbd> to
                  drag and <Kbd size="xs">Alt + RMB</Kbd> to scale and rotate. Paint through it to
                  project intricate patterns directly onto geometry.
                </p>
              </div>
              ...
              <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
                ...
                <p class="text-[11px] text-zinc-400">
                  Tap Space to open a radial menu right at your cursor. Flick your mouse towards any
                  tool wedge or color swatch for instant hands-free switching without moving to the
                  toolbar.
                </p>
              </div>
            </div>
        ...
        <Show when={activeTab() === 'pbr'}>
          <div class="flex flex-col gap-3.5 text-xs text-zinc-300 leading-relaxed">
            <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
              <h3 class="text-xs font-semibold text-zinc-100">
                What is Physically Based Rendering (PBR)?
              </h3>
              <p class="text-zinc-400 text-[11px]">
                PBR simulates real-world interactions between light rays and physical surfaces.
                Instead of baking lights or shadows into colors, surfaces are defined by independent
                physical attributes: Base Color, Roughness, Metalness, Normal maps, and Ambient
                Occlusion.
              </p>
            </div>
            ...
            <div class="space-y-2.5">
              <h3 class="text-xs font-bold text-zinc-200 uppercase tracking-wider">
                The 5 Core PBR Channels
              </h3>

              <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                <div class="flex items-center justify-between">
                  <span class="font-semibold text-zinc-200">Base Color (Albedo)</span>
                  <span class="text-[10px] px-2 py-0.5 rounded bg-blue-950 text-blue-400 font-mono">
                    sRGB
                  </span>
                </div>
                <p class="text-[11px] text-zinc-400">
                  Represents the pure diffuse color of dielectrics (plastics, wood, cloth) or the
                  specular reflectance tint of raw metals. It should never contain lighting,
              ...
              <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-[10px] px-2 py-0.5 rounded bg-zinc-800 text-zinc-300 font-mono">
                    Linear (0.0 - 1.0)
                  </span>
                </div>
                <p class="text-[11px] text-zinc-400">
                  Controls microsurface irregularity. A value of{' '}
                  <code class="text-zinc-200">0.0</code> creates a mirror-glossy reflection, while{' '}
                  <code class="text-zinc-200">1.0</code> produces completely diffuse, matte
                  reflections with broad light scattering.
                </p>
              </div>
              ...
              <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                ...
                <p class="text-[11px] text-zinc-400">
                  Defines whether the surface is non-metallic/dielectric (
                  <code class="text-zinc-200">0.0</code>) or raw conductive metal (
                  <code class="text-zinc-200">1.0</code>). Metals have zero diffuse reflection and
                  tint their specular highlights using the Base Color.
                </p>
              </div>
              ...
              <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-[10px] px-2 py-0.5 rounded bg-purple-950 text-purple-400 font-mono">
                    Tangent Space RGB
                  </span>
                </div>
                <p class="text-[11px] text-zinc-400">
                  Perturbs the surface normal per texel to fake high-frequency geometric detail like
                  bevels, panel lines, bumps, and scratches without increasing vertex count.
                </p>
              </div>
              ...
              <div class="p-3 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-1.5">
                ...
                <p class="text-[11px] text-zinc-400">
                  Simulates contact shadows in crevices, folds, and recessed corners where ambient
                  environmental light struggles to reach.
                </p>
              </div>
            </div>
            ...
            <div class="p-3.5 bg-gradient-to-r from-purple-950/30 to-zinc-950/60 border border-purple-500/30 rounded-xl flex flex-col gap-2">
              ...
              <p class="text-[11px] text-zinc-400">
                MeshCoat can pack three channels into a single texture to optimize VRAM and draw
                calls for Unreal Engine, Unity, Godot, and Blender:
              </p>
        ...
        <Show when={activeTab() === 'layers'}>
          <div class="flex flex-col gap-3.5 text-xs text-zinc-300 leading-relaxed">
            ...
            <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
              ...
              <p class="text-zinc-400 text-[11px]">
                Layers stack from bottom to top. The topmost visible layer renders over underlying
                layers according to its opacity and blend mode.
              </p>
              <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-[10px] mt-1">
                ...
                <div class="p-2 rounded bg-zinc-900 border border-zinc-800 text-zinc-300">
                  <span class="font-semibold text-zinc-200">Multiply</span>: Darkens / dirt &
                  shadows
                </div>
                <div class="p-2 rounded bg-zinc-900 border border-zinc-800 text-zinc-300">
            ...
            <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
              ...
              <p class="text-zinc-400 text-[11px]">
                Attach a mask to any layer to selectively hide or reveal areas without deleting
                paint. Press <Kbd size="xs">X</Kbd> while editing a mask to instantly swap between
                White (Reveal) and Black (Hide).
              </p>
            </div>
            ...
            <div class="p-3.5 bg-gradient-to-r from-amber-950/30 to-zinc-950/60 border border-amber-500/30 rounded-xl flex flex-col gap-2">
              ...
              <p class="text-zinc-400 text-[11px]">
                Analyzes mesh geometric curvature to generate realistic chipped edges, worn paint on
                sharp corners, and grime in recessed crevices. Tune Radius, Contrast, Noise Scale,
                and Threshold with live viewport feedback, then commit as a new layer or mask.
              </p>
            </div>
            ...
            <div class="p-3.5 bg-zinc-950/60 border border-zinc-800 rounded-xl flex flex-col gap-2">
              ...
              <p class="text-zinc-400 text-[11px]">
                When painting complex characters or vehicles containing multiple pieces (e.g. Head,
                Torso, Gear), click the{' '}
                <span class="text-zinc-200 font-medium">Isolate Active Piece</span> button in the
                top bar to hide all other meshes. This lets you work inside tight crevices without
                obstruction. Switching pieces via <Kbd size="xs">Tab</Kbd> or the layer panel
                dropdown automatically isolates the newly active piece!
              </p>
            </div>
          </div>
        </Show>
